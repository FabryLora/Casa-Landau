...
export default function MaterialesAdminRow({ material }) {
    ...
    const handleUpdate = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        updateForm.post(route('admin.materiales.update'), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const deleteMaterial = () => {
        if (confirm('¿Estas seguro de eliminar este material?')) {
            updateForm.delete(route('admin.materiales.destroy'), {
                preserveScroll: true,
                onSuccess: () => {
    ...
    return (
        <tr className={`border text-black odd:bg-gray-100 even:bg-white`}>
            <td className="align-middle">{material?.order}</td>
            <td className="h-[90px] align-middle">{material?.name}</td>
            ...
            <td className="w-[140px] text-center">
                <div className="flex flex-row justify-center gap-3">
                    ...
                    <button onClick={deleteMaterial} className="h-10 w-10 rounded-md border border-red-500 px-2 py-1 text-white">
                    ...
                    >
                        <form onSubmit={handleUpdate} method="POST" className="text-black">
                            <div className="w-[500px] rounded-md bg-white p-4">
                                <h2 className="mb-4 text-2xl font-semibold">Actualizar Material</h2>

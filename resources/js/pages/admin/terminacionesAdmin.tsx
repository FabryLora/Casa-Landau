import TerminacionesAdminRow from '@/components/terminacionesAdminRow';
...
export default function TerminacionesAdmin() {
    const { terminaciones } = usePage().props;
    ...
    const handleSubmit = (e) => {
        ...
        post(route('admin.terminaciones.store'), {
            preserveScroll: true,
            onSuccess: () => {
                toast.success('Terminación creada correctamente');
                reset();
                setCreateView(false);
            },
            onError: (errors) => {
                toast.error('Error al crear terminación');
                console.log(errors);
            },
        });
    };
        ...
        <Dashboard>
            <div className="flex w-full flex-col p-6">
                        ...
                        >
                            <form onSubmit={handleSubmit} method="POST" className="text-black">
                                <div className="w-[500px] rounded-md bg-white p-4">
                                    <h2 className="mb-4 text-2xl font-semibold">Crear Terminación</h2>
                ...
                <div className="mx-auto flex w-full flex-col gap-3">
                    <h2 className="border-primary-orange text-primary-orange text-bold w-full border-b-2 text-2xl">Terminaciones</h2>
                    <div className="flex h-fit w-full flex-row gap-5">
                        <input
                            type="text"
                            placeholder="Buscar terminación..."
                            value={searchTerm}
                    ...
                    <div className="flex w-full justify-center">
                        <table className="w-full border text-left text-sm text-gray-500 rtl:text-right dark:text-gray-400">
                            ...
                            <tbody className="text-center">
                                {terminaciones.data?.map((terminacion) => <TerminacionesAdminRow key={terminacion.id} terminacion={terminacion} />)}
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div className="mt-4 flex justify-center">
                        {terminaciones.links && (
                            <div className="flex items-center">
                                {terminaciones.links.map((link, index) => (
                                    <button
                                        key={index}
                                        ...
                                        } ${index === 0 ? 'rounded-l-md' : ''} ${index === terminaciones.links.length - 1 ? 'rounded-r-md' : ''}`}
                    ...
                    <div className="mt-2 text-center text-sm text-gray-600">
                        Mostrando {terminaciones.from || 0} a {terminaciones.to || 0} de {terminaciones.total} resultados
                    </div>
                </div>
            </div>
        </Dashboard>
